...
class ChannelDelete extends Event {
    ...
    async onLoad(channel) {
        ...
        let guardlog = await channel.guild.channels.cache.find(x => x.name == "guard-log").send({
            components: [gorevler],
            content: `Sunucuda kanal silindi! ${entry.executor} (\`${entry.executor.id}\`) tarafından kanal silindi. Kanal tekrar açıldı ve Üye ${member.bannable ? "banlandı!" : "banlanamadı!"}`
        })
