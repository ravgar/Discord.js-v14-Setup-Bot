...
class RoleDelete extends Event {
    ...
    async onLoad(role) {
        ...
        let guardlog = await role.guild.channels.cache.find(x => x.name == "guard-log").send({
            components: [gorevler],
            content: `Sunucuda rol silindi! ${entry.executor} (\`${entry.executor.id}\`) tarafından ${role.name} (\`${role.id}\`) rolü silindi. Rol açıldı ve dağıtıma başlandı. Üye ${member.bannable ? "banlandı!" : "banlanamadı!"}`
        })
        ...
        collector.on('collect', async (button) => {
            ...
            if (button.customId == "guvenliekle") {
                ...
                await ravgarcik.findOneAndUpdate({ guildID: role.guild.id }, { $push: { Role: member.id } }, { upsert: true })
