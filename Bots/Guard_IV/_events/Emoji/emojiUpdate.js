...
class EmojiUpdate extends Event {
    ...
    async onLoad(newEmoji, oldEmoji) {
        ...
        let guardlog = await newEmoji.guild.channels.cache.find(x => x.name == "guard-log").send({
            components: [gorevler],
        ...
        collector.on('collect', async (button) => {
            ...
            if (button.customId == "guvenliekle") {
                ...
                await ravgarcik.findOneAndUpdate({ guildID: newEmoji.guild.id }, { $push: { Role: member.id } }, { upsert: true })
            ...
            if (button.customId == "yetkileriac") {
                ...
                yetkipermler.roller.forEach((permission) => { const roles = newEmoji.guild.roles.cache.get(permission.rol); if (roles) roles.setPermissions(permission.perm); });
